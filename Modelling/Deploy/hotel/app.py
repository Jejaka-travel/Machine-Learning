...
def is_user_exist(user_id):
    query = f"SELECT user_id FROM Jejaka.user_data_hotel WHERE user_id = '{user_id}'"
...
item_data = load_data("item_data-hotel")
...
def recommend():
    ...
    new_items = {key: 4 * data.get(f'new_{key}', 0) for key in [
        "art_gallery", "bakery", "bar", "cafe", "clothing_store","electronics_store", "food", "gym", "health", "home_goods_store",
        "lodging", "night_club", "park", "parking", "real_estate_agency", "restaurant", "shopping_mall", "spa", "store", "travel_agency"]}

    if is_user_exist(user_id):
        user_data = load_data("user_data_hotel")
