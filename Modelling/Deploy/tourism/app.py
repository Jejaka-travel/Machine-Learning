...
def is_user_exist(user_id):
    query = f"SELECT user_id FROM Jejaka.user_data_tourism WHERE user_id = '{user_id}'"
...
item_data = load_data("item_data-tourism")
...
def recommend():
    ...
    new_items = {key: 4 * data.get(f'new_{key}', 0) for key in [
        "amusement_park", "aquarium", "art_gallery", "cafe", "church", "food", "hindu_temple", "library", "local_government_office",
        "lodging", "mosque", "museum", "natural_feature", "park", "place_of_worship", "restaurant", "rv_park", "school", "shopping_mall",
        "store", "travel_agency", "zoo"]}

    if is_user_exist(user_id):
        user_data = load_data("user_data_tourism")
